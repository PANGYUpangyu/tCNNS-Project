...
drug_smile_dict = np.load("新数据/drug_onehot_smiles.npy", encoding="latin1").item()
drug_smile_dict1 = np.load("新数据/drug_onehot_smiles1.npy", encoding="latin1").item()
drug_cell_dict = np.load("新数据/drug_cell_interaction.npy", encoding="latin1").item()
drug_cell_dict1 = np.load("新数据/drug_cell_interaction1.npy", encoding="latin1").item()
cell_mut_dict = np.load("新数据/cell_mut_matrix.npy", encoding="latin1").item()
cell_mut_dict1 = np.load("新数据/cell_mut_matrix1.npy", encoding="latin1").item()

c_chars = drug_smile_dict["c_chars"]
c_chars1 = drug_smile_dict1["c_chars"]
drug_names = drug_smile_dict["drug_names"]
drug_names1 = drug_smile_dict1["drug_names"]
drug_cids = drug_smile_dict["drug_cids"]
drug_cids1 = drug_smile_dict1["drug_cids"]
canonical = drug_smile_dict["canonical"]
canonical1 = drug_smile_dict1["canonical"]
canonical = np.transpose(canonical, (0, 2, 1))
canonical1 = np.transpose(canonical1, (0, 2, 1))
cell_names = cell_mut_dict["cell_names"]
cell_names1 = cell_mut_dict1["cell_names"]
mut_names = cell_mut_dict["mut_names"]
mut_names1 = cell_mut_dict1["mut_names"]
cell_mut = cell_mut_dict["cell_mut"]
cell_mut1 = cell_mut_dict1["cell_mut"]

all_positions = drug_cell_dict["positions"]
a = list(all_positions.tolist())
np.random.shuffle(a)
all_positions1 = drug_cell_dict1["positions"]
b = list(all_positions1.tolist())
...
def load_data(batch_size, label_list, positions=np.array(a),positions1=np.array(b)):
    ...
    test1 = Batch(batch_size, value, drug_smile1, cell_mut1, test1_pos)
    return train, valid, test, test1
...
def load_partial_data(batch_size, label_list, ratio, positions=np.array(a)):
    size = positions.shape[0]
    len1 = int(size * ratio)
...
def load_empty_data(batch_size, label_list, positions=np.array(a)):
    value_shape = drug_cell_dict["IC50"].shape
    value = np.zeros((value_shape[0], value_shape[1], len(label_list)))
    for i in range(len(label_list)):
        value[ :, :, i ] = drug_cell_dict[label_list[i]]
...
def load_drug_blind_data(batch_size, label_list, positions=np.array(a)):
    value_shape = drug_cell_dict["IC50"].shape
    value = np.zeros((value_shape[0], value_shape[1], len(label_list)))
    for i in range(len(label_list)):
        value[ :, :, i ] = drug_cell_dict[label_list[i]]
...
def load_cell_blind_data(batch_size, label_list, positions=np.array(a)):
    value_shape = drug_cell_dict["IC50"].shape
    value = np.zeros((value_shape[0], value_shape[1], len(label_list)))
...
def load_tissue_data(batch_size, label_list, tissue_ids, positions=np.array(a)):
...
def load_padel_drug_data(batch_size, label_list, positions=np.array(a)):
...
def load_part_feature_data(batch_size, label_list, positions=np.array(a), mut_size=310, cna_size=425):
    size = positions.shape[0]
    len1 = int(size * 0.8)
    len2 = int(size * 0.9)
    mut_list = []
